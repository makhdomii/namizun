...
# Backup existing netplan configs to avoid conflicting definitions
find /etc/netplan -maxdepth 1 -type f -name '*.yaml' ! -name "$(basename "${NETPLAN_FILE}")" | while read -r existing; do
  backup="${existing}.bak"
  if [[ ! -f ${backup} ]]; then
    mv "${existing}" "${backup}"
    echo "Backed up existing netplan file ${existing} -> ${backup}"
  fi
done
